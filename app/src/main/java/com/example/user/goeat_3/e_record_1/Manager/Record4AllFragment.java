...
public class Record4AllFragment extends Fragment implements View.OnClickListener, View.OnTouchListener {
    //這頁是管理者所有訂單統計數量跟總金額的頁面
    //宣告
    private Record4SponsorActivity record4SponsorActivity;
    ...
    String result="";
    private String order_id,store_name,order_status;
...
//    private TextView btn_return;
    ...
    public void onAttach(Context context) {
        ...
        record4SponsorActivity = (Record4SponsorActivity) context;
        order_id = record4SponsorActivity.order_id;
        store_name= record4SponsorActivity.store_name;
        order_status= record4SponsorActivity.order_status;//這個 如果是"尚未結單" 把勾勾拿掉frag1的      如果是 "餐店送達" 把勾勾勾起來
    }
...
//            check_time=0;
        ...
        new Record4AllFragment.ccon().execute("orderingcontent", order_id);
        new Record4AllFragment.ccon().execute("SelectStore", store_name);
    }
...
//                        check_time += 1;
                    ...
                    Toast.makeText(record4SponsorActivity, "餐點已經更新送達", Toast.LENGTH_SHORT).show();

                }else {
                    new ccon().execute("update_order", order_id,"已經結單");
                    Toast.makeText(record4SponsorActivity, "餐點通知已經發給用戶摟 目前無法取消喔", Toast.LENGTH_SHORT).show();
                }
                break;
            case R.id.fab:   //撥打電話
                Log.v("hint", "call");
...
//                       Log.d("abc", "建立新數量"+meal.get(c).getCount());
        ...
        protected void onPostExecute(String result) {
            ...
            if(!result.equals("dontsetmoney")) {
                ...
                txt_Amount.setText(Total_money + "");
                Total_money = 0;
            }
        }
    }
